    ...
    function GaudioProcessor(mInfo) {
        ...
        _this.setupCore = function () {
            if (!_this.mInitCore.get()) {
                ...
                if (_this.mTrackIndex >= _this.mInfo.getSolFilePath().size()) {
                    _this.mTrackIndex = 0;
                }
                var metadata = _this.getByteArrayFromFilePath(_this.mInfo.getSolFilePath().get(_this.mTrackIndex));
                ...
                _this.mInfo.setParams(controlParams);
                _this.mSetupResult = _this.mSolMusicOne.setup(_this.mConfig, _this.mInfo.getParams());
            ...
            for (var i = 0; i < count; i++) {
                ...
                if (_this.mInitCore.get()) {
                    if (_this.mIsUpdated.get()) {
                        var result = _this.mSolMusicOne.update(_this.mInfo.getParams());

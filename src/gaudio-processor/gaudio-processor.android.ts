...
export class GaudioProcessor extends java.lang.Object {
  ...
  private setupCore = () => {
    if (!this.mInitCore.get()) {
      ...
      if (this.mTrackIndex >= this.mInfo.getSolFilePath().size()) {
        this.mTrackIndex = 0;
      }

      let metadata: any = this.getByteArrayFromFilePath(this.mInfo.getSolFilePath().get(this.mTrackIndex)); // byte[]
      ...
      this.mInfo.setParams(controlParams);
      this.mSetupResult = this.mSolMusicOne.setup(this.mConfig, this.mInfo.getParams());
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (this.mInitCore.get()) {
        if (this.mIsUpdated.get()) {
          const result: number = this.mSolMusicOne.update(this.mInfo.getParams());
